...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    fun colDetec(obs: Array<Any>) {
        ...
        if (obsShape == "square"){
            ...
            if (deltaX.pow(2) + deltaY.pow(2) < (ball.width / 2).toDouble().pow(2) ) {
                ...
                val xCi = if(ball.x < obsIV.x) obsIV.x else obsIV.x + obsIV.width
                val yCi = if(ball.y < obsIV.y) obsIV.y else obsIV.y + obsIV.height
    ...
    fun collisionDampener(dampeningFactor: Double) {
        val newSpeed = sqrt(xVelo.pow(2) + yVelo.pow(2))*dampeningFactor
        val angle = atan(yVelo.absoluteValue/xVelo.absoluteValue)
        xVelo = if (xVelo > 0) ((newSpeed * cos(angle)).toFloat()) else (-(newSpeed * cos(angle))).toFloat()
        yVelo = if (yVelo > 0) (newSpeed * sin(angle)).toFloat() else (-(newSpeed * sin(angle))).toFloat()
    }

    fun FdragAccel(): Double {
        val totalSpeed = sqrt(xVelo.pow(2) + yVelo.pow(2))
        val ballMass = ((4/3)*(3.14)*((diameter/2).toFloat().pow(3)))* ball_density
        val force =  (((0.5) * (medium_density) * (totalSpeed.pow(2))  * 0.000000001 *  ((3.14* (diameter/2)/100).pow(2))).toDouble())
